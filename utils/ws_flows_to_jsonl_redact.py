...
def approx_b64_bytes(s: str) -> int:
    return (len(s.rstrip("=")) * 3) // 4

if len(sys.argv) < 3:
    print("Usage: python ws_flows_to_jsonl_redact.py <flows-file> <out-jsonl> [host_filter] [path_prefix]")
    sys.exit(1)

flows_path, out_path = sys.argv[1], sys.argv[2]
        ...
        for msg in ws.messages:
            ...
            if isinstance(obj, dict) and "audio" in obj and isinstance(obj["audio"], str):
                ...
                obj["audio_info"] = {
                    "redacted": True,
                    "b64_len": len(b64),
                    "approx_bytes": approx_b64_bytes(b64),
                }
                del obj["audio"]

            # Optional: shrink alignment
            al = obj.get("alignment")
            if al:
                try:
                    obj["alignment"] = {
                        "count": len(al.get("chars", [])),
                        "t_first": (al.get("charStartTimesMs") or [None])[0],
                        "t_last": (al.get("charStartTimesMs") or [None])[-1],
                        "dur_sum": sum(al.get("charDurationsMs") or []),
                    }
                except Exception:
                    obj["alignment"] = {"redacted": True}
